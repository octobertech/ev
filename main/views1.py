...
class GoodView(object):
    ...
    def get_queryset(self):
        return Good.objects.published().select_related("user", "user__profile")


class GoodList(GoodView, ScoreOrderingView):
    """
    ...
    def get_title(self, context):
        ...
        if context["profile_user"]:
            return "Goods by %s" % context["profile_user"].profile
        else:
            return "Latest"
...
class GoodCreate(CreateView):
    ...
    form_class = modelform_factory(Good, fields=("title", "description", "Unit price"))
    model = Good

    def form_valid(self, form):
        titles = getattr(settings, "ALLOWED_DUPLICATE_GOOD_TITLES", None)
        if form.instance.title:
            if titles:
                ...
                try:
                    title = Good.objects.get(**lookup)
                except Good.DoesNotExist:
                    pass
                else:
                    error(self.request, "Good exists")
                    return redirect(good)
        form.instance.user = self.request.user
        form.instance.gen_description = False
        info(self.request, "Good created")
        return super(GoodCreate, self).form_valid(form)


class GoodDetail(GoodView, DetailView):
    """
